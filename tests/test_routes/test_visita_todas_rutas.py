    ...
    with app.app_context():
        ...
        for route in routes_to_test:
            try:
                ...
                if response.status_code >= 500:
                    errores.append(
                        f"Ruta {route}: Error {response.status_code} (Internal Server Error)"
                    )
                    ...
                    advertencias.append(
                        f"Ruta {route}: Código {response.status_code}"
                    )
...
def test_lista_todas_las_rutas_disponibles(app):
    ...
    with app.app_context():
        ...
        estadisticas = (
            f"Rutas GET: {len(rutas_get)}, "
            f"Rutas POST: {len(rutas_post)}, "
            f"Otras rutas: {len(rutas_otras)}"
        )

    ...
    with app.app_context():
        ...
        for inicio, fin, days in payroll_dates:
            amount_anual = calculate_prorated_prestacion(
                ...
                prestacion_anual.porcentaje, 
                days,
                tipo_planilla.dias
            )
            amount_vida = calculate_prorated_prestacion(
                empleado.salario_base,
                prestacion_vida.porcentaje,
                days,
                tipo_planilla.dias
            )
        ...
        for i, trans in enumerate(transacciones_anual):
            ...
            assert trans.monto_transaccion == expected_amount, \
                f"Transaction {i+1} amount should be {expected_amount}, got {trans.monto_transaccion}"
            
            # Verify running balance calculation
            assert trans.saldo_anterior == running_balance_anual, \
            ...
            assert trans.saldo_nuevo == running_balance_anual, \
        ...
        for i, trans in enumerate(transacciones_vida):
            ...
            # Verify running balance calculation
            assert trans.saldo_anterior == running_balance_vida, \
            ...
            assert trans.saldo_nuevo == running_balance_vida, \
        ...
        for trans in transacciones_anual + transacciones_vida:
            ...
            assert trans.nomina_id in [n.id for n in nominas_executed], \
        ...
        assert len(all_trans_ids) == len(set(all_trans_ids)), \
            "All transactions should have unique IDs (no updates)"
        ...
        print(f"   - All transactions properly linked to source nominas")
        print(f"   - Running balances calculated correctly")
...
def test_prestaciones_monthly_settlement(app, db_session):
    ...
    with app.app_context():
        ...
        for i, trans in enumerate(transacciones):
            assert trans.tipo_transaccion == "adicion", f"Transaction {i+1} should be 'adicion'"
            assert trans.monto_transaccion > Decimal("0"), \
                f"Transaction {i+1} should have positive amount"
            assert trans.saldo_anterior == Decimal("0.00"), \
                f"Transaction {i+1} should have 0 previous balance (monthly settlement)"
            assert trans.saldo_nuevo == trans.monto_transaccion, \
        ...
        print(f"   - Each month's balance resets to 0")

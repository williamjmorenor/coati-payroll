...
def test_admin_creates_hhrr_and_audit_users(client, app, db_session, admin_user):
    ...
    with app.app_context():
        ...
        assert len({db_admin.id, db_hhrr.id, db_audit.id}) == 3, \
    ...
    with app.app_context():
        ...
        for usuario, password, nombre, apellido, tipo in users_data:
            ...
            assert login_response.status_code in (302, 303), \
                f"User {usuario} should login successfully"
            
            # Verify can access home
            home_response = client.get("/")
            assert home_response.status_code == 200, \
                f"User {usuario} should access home page"
        ...
        inactive_by_name = db_session.query(Usuario).filter_by(
            usuario="hhrh_manager", activo=False
        ).count()
...
def test_user_type_segregation_and_permissions(client, app, db_session, admin_user):
    ...
    with app.app_context():
        ...
        for usuario, password, expected_tipo in test_credentials:
            ...
            assert db_user.tipo == expected_tipo, \
                f"User {usuario} should have type {expected_tipo}"
            
            # Logout
            logout_user(client)
        
        # Verify all created users are still active
        created_active = db_session.query(Usuario).filter(
            Usuario.usuario.in_(["admin2", "hhrr1", "hhrr2", "audit1", "audit2", "admin-test"])
        ).filter_by(activo=True).count()
